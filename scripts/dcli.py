...
def start_daemon():
    ...
    if not is_daemon_running():
        ...
        for _ in range(20):
            time.sleep(0.5)
            if SOCKET_PATH.exists() and read_auth_token():
                return True
...
def send_request(command_data, timeout=30):
    """Send request to daemon via the private Unix socket."""
    if not is_daemon_running():
        start_daemon()

    auth_token = read_auth_token()
    if not auth_token:
        print("Error: Daemon auth token not found. Is daemon running?")
        sys.exit(1)
    ...
    try:
        ...
        request_payload = dict(command_data)
        request_payload["auth"] = auth_token
        ...
        print(f"Error communicating with daemon: {exc}")
        sys.exit(1)
...
def cmd_read_messages(channel_id, limit, after=None):
    ...
    result = send_request({"command": "read_messages", "args": args})
    if "error" in result:
        print(f"Error: {result['error']}")
        return
...
def cmd_list_guilds():
    result = send_request({"command": "list_guilds", "args": {}})
    if "error" in result:
        print(f"Error: {result['error']}")
        return
...
def cmd_list_channels(guild_id):
    result = send_request({"command": "list_channels", "args": {"guild_id": guild_id}})
    if "error" in result:
        print(f"Error: {result['error']}")
        return
...
def cmd_list_guild_threads(guild_id):
    result = send_request(
        {"command": "list_guild_threads", "args": {"guild_id": guild_id}}
    )
    if "error" in result:
        print(f"Error: {result['error']}")
        return
...
def cmd_read_recent_threads(guild_id, within_hours, limit_per_thread):
    ...
    print(
        f"Recent threads ({len(threads)}) with {total} messages in last {within_hours}h:"
    )
    print("=" * 60)

    for thread in threads:
        print(f"\nThread {thread['name']} (ID: {thread['id']}, Parent: #{thread['parent']})")
        print("-" * 60)
...
def cmd_read_thread(thread_id, limit, after=None):
    ...
    result = send_request({"command": "read_thread", "args": args})
    if "error" in result:
        print(f"Error: {result['error']}")
        return
...
def cmd_get_thread_info(thread_id):
    result = send_request({"command": "get_thread_info", "args": {"thread_id": thread_id}})
    if "error" in result:
        print(f"Error: {result['error']}")
        return
...
def cmd_join_thread(thread_id):
    result = send_request({"command": "join_thread", "args": {"thread_id": thread_id}})
    if "error" in result:
        print(f"Error: {result['error']}")
        return
...
def cmd_leave_thread(thread_id):
    result = send_request({"command": "leave_thread", "args": {"thread_id": thread_id}})
    if "error" in result:
        print(f"Error: {result['error']}")
        return
...
def cmd_user_info(user_id):
    result = send_request({"command": "user_info", "args": {"user_id": user_id}})
    if "error" in result:
        print(f"Error: {result['error']}")
        return
